...
import { InputMask } from '../../data/inputs'
...
export function Contato({ api }: IContatoProps) {
  ...
  return (
    <Form ref={formRef} onSubmit={handleSubmit}>
      ...
      <InputMask name='nome' type='text' mask={''} />

      <InputMask name='email' type='email' mask={''} />

      <InputMask name='telefone' type='text' mask={''} />

      <button type='submit'>
        {stateForm.state === 'loading' ? <LoaderCircle size={40} color={'var(--secundary-color)'} /> : 'Enviar'}
      </button>
    </Form>
  )
}
